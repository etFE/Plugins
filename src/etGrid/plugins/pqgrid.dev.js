...
(function ($) {
	...
	_pGenerateView._renderCell = function (objP, _dataCell) { // 此方法已在下面重写 修改复选框渲染
		var that = this.that,
			options = this.options,
	...
	_pGenerateView._generateRow = function (rowData, rowIndx, CM, buffer, objP, lastFrozenRow, nestedshow) {
		...
		/***********给内置数据赋行索引 ---(cl)************* */
		rowData['_rowIndx'] = rowIndx + offset;
		rowData['_rowIndxPage'] = rowIndx;
		/******************************************* */
		var objRender = {
	...
	fn._refreshGridWidthAndHeight = function () {
		...
		if (widthPercent || heightPercent) {
			...
			var wdParent = parent.width(),
				//	htParent = (parent[0] == document.body) ? $(window).height() : parent.height(),
				htParent = (parent[0] == document.body || o.inWindowHeight) ? $(window).height() : parent.height(), //  以上是原版本  已改过 ---(cl)
				superParent = null,
			...
			for (var dataIndx in newRow) {
				...
				if (column) {
					if (rowCheckEditable && column.editable != null && (that.isEditableCell({
						...
						// delete newRow[dataIndx];    ---(cl) //  注释 因为添加行时影响到数据不回充
						continue
					}
	...
	fn.getCellHeader = function (obj) {
		...
		if ($tbl != undefined) {
			...
			/*******以上是原版本 因表格是多行表头时 渲染时表头复选框没有渲染出来 所以修改为下  --(cl) */
			var $td = $tbl.children().children("tr.pq-grid-title-row:first").children("td[pq-col-indx=" + colIndx + "]");
		}
	...
	_pKeyNav._incrEditIndx = function (rowIndxPage, colIndx) { //  返回下一个可编辑的单元格对象
		var that = this.that,
			CM = that.colModel,
			CMLength = CM.length,
	...
	_pKeyNav._keyDownInEdit = function (evt) { // 方法已重写
		var that = this.that,
			o = that.options;
		var EMIndx = o.editModel.indices;
		if (!EMIndx) {
		...
		if (evt.keyCode == keyCodes.LEFT) {
			...
			if (evt.keyCode == keyCodes.RIGHT) {
				...
				if (evt.keyCode == keyCodes.UP) {
					...
					if (evt.keyCode == keyCodes.DOWN) {
						//----------add (cl)----------------
						if (thisOptions.addRowByKey) {
							if (rowIndx == dataLastIndx) { // 使光标停留在最后一行的最后的可编辑列中
								that.addRow({
									rowData: {}
								});
							}
...
(function ($) {
	...
	_pUCData.add = function (obj) { // 此方法已重写 ---(cl)
		var that = this.that,
			adata = this.adata,
			rowData = obj.rowData,
	...
	_pUCData.update = function (objP) { //  此方法已重写  
		var that = this.that,
			TM = this.options.trackModel,
			dirtyClass = TM.dirtyClass,
...
(function ($) {
	...
	_pR.extendSelection = function (objP) { // 行连选   此方法已重写   
		var that = this.that,
			rowIndx = objP.rowIndx,
	...
	_pC.extendSelection = function (objP) { // 单元格连选   此方法已重写   
		var that = this.that,
	...
	_pR.selectAll = function (objP) { //  所有行全选方法  (此方法已重写)
		var that = this.that,
			all = (objP && objP.all) ? true : false,
			data = all ? this.options.dataModel.data : that.data,
			rows = [];
...
(function ($) {
	...
	_pMouseSelection._onCellMouseDown = function (evt, ui) { //  此方法已重写  
		var that = this.that,
			rowIndx = ui.rowIndx,
			colIndx = ui.colIndx,
			SM = that.options.selectionModel,
	...
	_pMouseSelection._onRowMouseDown = function (evt, ui) { //  此方法已重写 
		var that = this.that,
			self = this,
			rowIndx = ui.rowIndx,
			SM = that.options.selectionModel,
