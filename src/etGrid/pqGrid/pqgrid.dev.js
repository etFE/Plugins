...
(function($) {
	...
	fn._create = function() {
		...
		// $cont.on("click", 'td.pq-grid-cell .et-checkBox', function(evt) {
		// 	evt.stopPropagation();
		// });
	...
	fn._refreshGridWidthAndHeight = function() {
		...
		if (widthPercent || heightPercent) {
			...
			var wdParent = parent.width(),
				htParent = (parent[0] == document.body || o.inWindowHeight) ? $(window).height() : parent.height(),  // 原版本是$(document).height()  已改过 ---(cl)
				superParent = null,
			...
			if (heightPercent) {
				...
				ht -= element.offset().top;    // 让高度自适应 ---(cl)
			}
			...
			for (var dataIndx in newRow) {
				...
				if (column) {
					if (rowCheckEditable && column.editable != null && (that.isEditableCell({
						...
						// delete newRow[dataIndx];    ---(cl) //  暂时先注释 因为添加行时影响到数据不回充
						continue
					}
	...
	_pR.selectAll = function(objP) {                         //  所有行全选方法
		...
		if (all) {
			...
			for (var i = 0, len = data.length; i < len; i++) {
				...
				if(data[i].disabledcheckbox) {                                 //  disabledcheckbox有值时不会被选择(cl)
					continue;
				}
				var obj = {
					rowData: data[i],
